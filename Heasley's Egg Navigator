...
//`键切换页面
let isKeydown = false; // 用于标记按键状态

window.onkeydown = function (event) {
    if (event.keyCode !== 192) return; // 仅检测`键
    if (event.repeat || isKeydown) return; // 如果是重复触发或标志已设置，直接返回

    isKeydown = true; // 标记按键已按下
    ...
    localStorage.setItem("eeh-index", linkIndex);
    $('#eggTraverse').attr('href', EVERY_LINK[linkIndex]);
    window.location.href = "https://www.torn.com/" + EVERY_LINK[linkIndex];
};

window.onkeyup = function (event) {
    if (event.keyCode === 192) { // 检测`键释放
        isKeydown = false; // 重置标记为未按下状态
    }
};
...
(function() {
    ...
    // 非浏览器直接退出，防止 Node 环境报错
    if (typeof document === 'undefined') return;

    // 配置
    const SELECTOR = '#easter-egg-hunt-root button';
    ...
    if (!document.getElementById('egg-style')) {
        ...
        style.textContent = `
            ${SELECTOR} {
                ...
                animation: pop 0.4s cubic-bezier(0.3,1.6,0.6,1) !important;
    ...
    // 处理单个彩蛋
    function initEgg(btn) {
        if (btn.dataset.eggInit) return;
        btn.dataset.eggInit = 'true';
    ...
    // 自动查找并监听彩蛋
    function watchEggs() {
        document.querySelectorAll(SELECTOR).forEach(initEgg);
    }

    // 监听页面变化
    if (typeof MutationObserver !== 'undefined') {
        new MutationObserver(watchEggs).observe(document.body, {
            childList: true,
            subtree: true
        });
    }
